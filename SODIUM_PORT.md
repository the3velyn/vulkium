...
- **2026-06-28** Optimisation continuation pass:
  ...
  - **`1ad2dd7` — translucent sort cancel** (later reverted, see `88434b1`). Initial attempt: HEAD-cancel `RenderSectionManager#scheduleSort` to save Sodium worker CPU. The user flagged this as the wrong default per the "use Sodium's features rather than cancel" guideline — vulkium should consume Sodium's sort output instead of skipping it.
  ...
  - **`dcbf2b3` — Stage 5 sodium GUI.** `VideoSettingsScreenMixin` (sodium-side) adds a "Vulkium Options..." button at the bottom-left of Sodium's settings screen, parallel to the existing vanilla-side mixin's button on MC's screen.
  - **`18d145d` — Stage 6.3 final mixin cleanup.** Deleted `ChunkSectionsToRenderMixin` (redundant cancel — Sodium already cancels MC's vanilla terrain path), `MojangColorFormat` (its capture data was a closed loop with no real consumer), and `RenderSectionMixin` (already a no-op placeholder). vulkium.mixins.json down to 7 entries.
  - **`88434b1` — sort cancel reverted, sort consumption added.** Pulled `1ad2dd7` per "use Sodium's features" guideline. New `ChunkBuilderSortingTaskMixin` captures Sodium's `ChunkSortOutput.getSorter().getIndexBuffer()` (uint32, 6 per quad, [4k+0,4k+1,4k+2,4k+2,4k+3,4k+0] layout) and queues it via the standalone-era `SectionManager.offerResort` → `TerrainUploader.resortTranslucent` path. POV-sorted water/glass now Just Works under sodium.
...
| Translucent per-quad sort tasks (`ChunkBuilderSortingTask`) | medium CPU (only when camera moves through translucent threshold) | sort cancel reverted per "use Sodium's features" guideline | ✅ consumed (`ChunkBuilderSortingTaskMixin` → `SectionManager.offerResort` → `TerrainUploader.resortTranslucent`) |
